    ...
    BEGIN
        FOR trains_from IN (SELECT * FROM routes WHERE station_id = from_station) LOOP
            FOR trains_to IN (SELECT * FROM routes WHERE station_id = to_station) LOOP
                IF trains_from.train_id = trains_to.train_id AND ((trains_from.arr_date<trains_to.arr_date) OR (trains_from.arr_date = trains_to.arr_date AND trains_from.arr_time < trains_to.arr_time)) THEN
                ...
                FOR intermediate_stations_from IN (SELECT * FROM routes WHERE train_id = trains_from.train_id) LOOP
                    FOR intermediate_stations_to IN (SELECT * FROM routes WHERE train_id = trains_to.train_id) LOOP
                        IF intermediate_stations_from.station_id = intermediate_stations_to.station_id AND ((trains_from.arr_date < intermediate_stations_from.arr_date) OR (trains_from.dep_time < intermediate_stations_from.arr_time)) AND ((intermediate_stations_from.arr_date < intermediate_stations_to.dep_date) OR (intermediate_stations_from.arr_time < intermediate_stations_to.dep_time)) AND ((intermediate_stations_to.arr_date < trains_to.arr_date) OR (intermediate_stations_to.dep_time < trains_to.arr_time)) THEN
                            station_1_id := from_station;
                            station_1_arr_time := trains_from.arr_time;
                            station_1_dep_time := trains_from.dep_time;
                            station_1_dep_date := trains_from.dep_date;
                            ...
                            station_2_arr_date := trains_to.arr_date;
                            station_2_dep_date := trains_to.dep_date;
